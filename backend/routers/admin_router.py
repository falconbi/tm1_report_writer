...
async def get_stats(session: Session = Depends(get_session)):
    ...
    draft_count      = session.exec(select(func.count()).select_from(Report).where(Report.status == "draft")).one()
    published_count  = session.exec(select(func.count()).select_from(Report).where(Report.status == "published")).one()
    dirty_count      = session.exec(select(func.count()).select_from(Report).where(Report.has_draft == True)).one()
